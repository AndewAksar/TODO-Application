...
Этот roadmap описывает будущие этапы и историческую последовательность работ,
а не текущий runtime. Текущее состояние документировано в `README.md` и
`docs/architecture/`. Шаги 1–6, включая JWT и user-owned Tasks CRUD, уже
реализованы; Kafka/event, scheduler, mailer и frontend остаются будущими
этапами, пока соответствующие task records не получат `Status: Completed`.
...
## Разработка, управляемая задачами

Подробные пошаговые задачи разработки отслеживаются в файле:

- `docs/tasks/000-index.md`

Каждая задача определяет область применения, ограничения и критерии приемки.
Задачи разработаны таким образом, чтобы быть небольшими, поэтапными и управляемыми системой непрерывной интеграции (CI).
...
- Интеграционные тесты для инфраструктурно-зависимого поведения.
...
**Подзадачи**
- Полный `README.md`.
- актуализировать `docs/architecture/`.
- актуализировать `docs/contracts/events/`.
- актуализировать `docs/api/`.
